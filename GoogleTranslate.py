...
def get_translate(inputtext, sourcelanguage='auto',targetlanguage='tr'):
    global nottk
    urltext = quote(inputtext)
    sl = sourcelanguage
    tl = targetlanguage

    url = f"https://translate.google.com.tw/translate_a/single?&client=gtx&dt=t&sl={sl}&tl={tl}&hl={tl}&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=sos&dt=ss&dt=t&source=bh&ssel=0&tsel=0&xid=1791807&kc=1&q={urltext}"

    if not nottk:
        tkid = tk.calculate_token(inputtext)
        url=f"https://translate.google.com/translate_a/single?&client=webapp&dt=t&sl={sl}&tl={tl}&hl={tl}&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=sos&dt=ss&dt=t&source=bh&ssel=0&tsel=0&xid=1791807&kc=1&q={urltext}&tk={tkid}"
